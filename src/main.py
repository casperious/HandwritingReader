...
if __name__ == "__main__":
    ...
    if args.transform:
        ...
        predicts, probabilities = model.predict(
            x_test, ctc_decode=True
        )  # ctc_decode=True was original
        print(predicts, "\n", probabilities)
        ...
        if args.kaldi_assets:
            ...
            sufix = (
                ("_norm" if args.norm_accentuation or args.norm_punctuation else "")
                + ("_accentuation" if args.norm_accentuation else "")
                + ("_punctuation" if args.norm_punctuation else "")
            )

            with open(
                os.path.join(output_path, f"evaluate_kaldi{sufix}.txt"), "w"
            ) as lg:
                lg.write(e_corpus)
                print(e_corpus)
